...
namespace Jg.wpf.controls.Customer.CoverFlowControl
{
    public class CoverFlowItem : ModelVisual3D
    {
        ...
        public UIElement Element => _uiElement;
        ...
        public CoverFlowItem(int itemIndex, int currentIndex, UIElement element)
        {
            ...
            this.Children.Add(new Viewport2DVisual3D()
            {
                Visual = _uiElement,
                Geometry = CreateItemGeometry(),
                Material = new DiffuseMaterial(new VisualBrush(_uiElement)),
        ...
        private double GetZByPos(int index) => 1 - Math.Abs(Index - index);
