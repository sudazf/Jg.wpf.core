...
namespace Jg.wpf.controls.Customer.FastDataGrid.Controls
{
    ...
    public partial class FastGridControl : UserControl, IFastGridView
    {
        ...
        public FastGridControl()
        {
            ...
            //gridCore.Grid = this;
            CellFontSize = 11;
            RowHeaderVisibility = Visibility.Visible;
            ...
            PreviewDrop += FastGridControl_PreviewDrop;
        }
        ...
        private void RecalculateDefaultCellSize()
        {
            ...
            _rowSizes.DefaultSize = rowHeight + 20;
            _columnSizes.DefaultSize = columnWidth + 20;
        ...
        {
            if (RowHeaderVisibility == Visibility.Visible)
            {
        ...
        private void FixCurrentCellAndSetSelectionToCurrentCell()
        {
            ...
            OnChangeSelectedCells(false, row, col);
        }
        ...
        {
            var actualRow = _rowSizes.RealToModel(row);
            var actualCol = _columnSizes.RealToModel(col);

            if (IsTransposed)
            {
                actualRow = _columnSizes.RealToModel(col);
                actualCol = _rowSizes.RealToModel(row);
            }

            return GetModelCell(actualRow, actualCol);
        }
        ...
        public void HideInlineEditor(bool saveCellValue = true)
        {
            using (var ctx = CreateInvalidationContext())
            {
                if (saveCellValue && _inplaceEditorCell.IsCell && _inlineTextChanged)
                {
                    ...
                    if (selectOpera.Visibility == Visibility.Visible)
                    {
                        cell.SetEditText(selectOpera.SelectedIndex == 0 ? "And" : "Or", EditCellType.ComboBoxOpera);
                    }
                    else if (selectColor.Visibility == Visibility.Visible)
                    {
                        var selectedColor = selectColor.SelectedItem as JColor;
                        cell.SetEditText(selectedColor, EditCellType.ComboBoxColor);
                    }
                    else if (selectColorText.Visibility == Visibility.Visible)
                    {
                        var selectedColor = selectColorText.SelectedItem as JColor;
                        var text = selectColorText.Display;
                        cell.SetEditText(text, selectedColor, EditCellType.SelectableColorText);
                    }
                    else if (edText.Visibility == Visibility.Visible)
                    {
                ...
                selectOpera.Visibility = Visibility.Hidden;
                selectColor.Visibility = Visibility.Hidden;
                selectColorText.Visibility = Visibility.Hidden;
            }
        ...
        private void ShowInlineEditor(FastGridCellAddress cell, string textValueOverride = null)
        {
            ...
            var cellType = _model.GetCellType(cell.Row.Value, cell.Column.Value);
            var cellValue = _model.GetCellValue(cell.Row.Value, cell.Column.Value);
            ...
            switch (cellType)
            {
                case EditCellType.TextBox:
                    ...
                    break;
                case EditCellType.ComboBoxOpera:

                    selectOpera.Visibility = Visibility.Visible;
                    _inplaceEditorCell = cell;


                    selectOpera.Margin = new Thickness
                    {
                    ...
                    selectOpera.Text = text;

                    break;
                case EditCellType.ComboBoxColor:
                    selectColor.Visibility = Visibility.Visible;
                    selectColor.Margin = new Thickness
                    {
                        Left = rect.Left / DpiDetector.DpiXKoef,
                        Top = rect.Top / DpiDetector.DpiYKoef,
                        Right = imageGrid.ActualWidth - rect.Right / DpiDetector.DpiXKoef,
                    ...
                    if (cellValue.Value is JColor color)
                    {
                        selectColor.SelectedItem = JColors.Source.FirstOrDefault((c) =>
                            c.R == color.R && c.G == color.G && c.B == color.B);
                    }
                    break;
                case EditCellType.SelectableColorText:
                    selectColorText.Visibility = Visibility.Visible;
                    selectColorText.Margin = new Thickness
                    {
                        Left = rect.Left / DpiDetector.DpiXKoef,
                        Top = rect.Top / DpiDetector.DpiYKoef,
                        Right = imageGrid.ActualWidth - rect.Right / DpiDetector.DpiXKoef,
                    ...
                    selectColorText.Display = cellValue.Display;
                    selectColorText.SelectDisplay();

                    if (cellValue.Value is JColor colorText)
                    {
                        var items = selectColorText.Items.Cast<JColor>();
                        var item = items.FirstOrDefault(c =>
                            c.R == colorText.R && c.G == colorText.G && c.B == colorText.B);
                        selectColorText.SelectedItem = item;
                    }
        ...
        private void AdjustInlineEditorPosition()
        {
            if (_inplaceEditorCell.IsCell)
            {
                ...
                var cellType = _model.GetCellType(_inplaceEditorCell.Row.Value, _inplaceEditorCell.Column.Value);
                var rect = GetCellRect(_inplaceEditorCell.Row.Value, _inplaceEditorCell.Column.Value);
                ...
                switch (cellType)
                {
                    case EditCellType.TextBox:
                        ...
                        edText.Margin = margin;
                        break;
                    case EditCellType.ComboBoxOpera:
                        selectOpera.Visibility = visible ? Visibility.Visible : Visibility.Hidden;
                        selectOpera.Margin = margin;
                        break;
                    case EditCellType.ComboBoxColor:
                        selectColor.Visibility = visible ? Visibility.Visible : Visibility.Hidden;
                        break;
                    case EditCellType.SelectableColorText:
                        selectColorText.Visibility = visible ? Visibility.Visible : Visibility.Hidden;
                        selectColorText.Margin = margin;
                        break;
                    default:
                        throw new ArgumentOutOfRangeException();
                }
        ...
        private bool MoveCurrentCell(int? row, int? col, KeyEventArgs e = null)
        {
            ...
            OnChangeSelectedCells(true, row, col);
            return true;
        }
        ...
        private void selectDropdownClosed(object sender, EventArgs e)
        {
            using (var ctx = CreateInvalidationContext())
            {
                HideInlineEditor();
        ...
        private void selectText_SelectionChanged(object sender, System.Windows.Controls.SelectionChangedEventArgs e)
        {
            _inlineTextChanged = true;
        }

        private void selectColor_SelectionChanged(object sender, System.Windows.Controls.SelectionChangedEventArgs e)
        {
            _inlineTextChanged = true;
        }

        private void OnCustomDisplayChanged(object sender, DisplayChangedEventArgs e)
        {
            _inlineTextChanged = true;
        }
    }
