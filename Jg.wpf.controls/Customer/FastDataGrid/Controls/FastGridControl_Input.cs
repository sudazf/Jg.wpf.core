...
namespace Jg.wpf.controls.Customer.FastDataGrid.Controls
{
    ...
    {
        public event EventHandler<DropEventArgs> PreviewsDropChanged;
        ...
        protected override void OnMouseLeftButtonDown(System.Windows.Input.MouseButtonEventArgs e)
        {
            ...
            using (var ctx = CreateInvalidationContext())
            {
                    ...
                    && (_lastDblClickResize == null || DateTime.Now - _lastDblClickResize.Value > TimeSpan.FromSeconds(1)))
                {
                    ...
                    OnChangeSelectedCells(true, cell.Row, cell.Column);
                }
        ...
        protected override void OnMouseRightButtonDown(MouseButtonEventArgs e)
        {
            ...
            OnRightClickCell(cell.Row, cell.Column, cell.IsRowHeader, cell.IsColumnHeader);
        }
        ...
        private void imageKeyDown(object sender, KeyEventArgs e)
        {
            using (var ctx = CreateInvalidationContext())
            {
                ...
                {
                    var selectedCells = GetSelectedModelCells();
                    if (selectedCells.Count > 0)
                    {
                        var minRowIndex = 9999;
                        var minColumnIndex = 9999;
                        foreach (var cellAddress in selectedCells)
                        {
                            if (cellAddress.IsCell)
                            {
                                if (minRowIndex > cellAddress.Row.Value)
                                {
                                    minRowIndex = cellAddress.Row.Value;
                                }
                                if (minColumnIndex > cellAddress.Column.Value)
                                {
                                    minColumnIndex = cellAddress.Column.Value;
                                }
                            }
                        }

                        var selectedRows = GetActiveRealRows().Selected.Count;
                        var selectedColumns = GetActiveRealColumns().Selected.Count;
                        ...
                        for (int i = 0; i < selectedRows; i++)
                        {
                            ...
                            {
                                var columnContent = Model.GetCellValue(minRowIndex + i, minColumnIndex + j);
                                rowContent += $"{columnContent.Display} ";
                            }
                ...
                if (e.Key == Key.V && ControlPressed)
                {
                    ...
                    if (selectedCells.Count > 0)
                    {
                        ...
                        {
                            var copiedBinNames = copied.Split('\n');
                            for (int i = 0; i < copiedBinNames.Length; i++)
                            {
                                var cell = Model.GetCell(this, minRowIndex + i, minColumnIndex);
                                cell.SetEditText(copiedBinNames[i].Trim('\r').Trim(), EditCellType.SelectableColorText);
                                InvalidateCell(minRowIndex + i, minColumnIndex);
                            }
                        }
                    }
                }
        ...
        private void OnDrop(DragEventArgs e, int? row, int? col, bool isRowHeader, bool isColumnHeader)
        {
            PreviewsDropChanged?.Invoke(this, new DropEventArgs(e, row, col, isRowHeader, isColumnHeader));
        }

        private void FastGridControl_PreviewDrop(object sender, DragEventArgs e)
        {
            var pt = e.GetPosition(image);
            pt.X *= DpiDetector.DpiXKoef;
            pt.Y *= DpiDetector.DpiYKoef;
            ...
            OnDrop(e, cell.Row, cell.Column, cell.IsRowHeader, cell.IsColumnHeader);
        }
    }
}
