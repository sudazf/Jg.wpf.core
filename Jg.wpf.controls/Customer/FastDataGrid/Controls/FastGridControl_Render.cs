        ...
        private int GetCellContentHeight(IFastGridCell cell)
        {
            ...
            for (int i = 0; i < cell.BlockCount; i++)
            {
                ...
                if (block.BlockType != FastGridBlockType.Text &&
                    block.BlockType != FastGridBlockType.ColorText)
                {
                    continue;
                }
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                switch (block.BlockType)
                {
                    case FastGridBlockType.Text:
                        ...
                        witdh += font.GetTextWidth(text, maxSize) + 32; //add by ji
                        break;
                    case FastGridBlockType.Image:
                        witdh += block.ImageWidth;
                        break;
                    case FastGridBlockType.Color:
                        witdh += 80;
                        break;
                    case FastGridBlockType.ColorText:
                        string colorText = block.TextData;
                        var colorTextFont = GetFont(block.IsBold, block.IsItalic);
                        witdh += colorTextFont.GetTextWidth(colorText, maxSize) + 100; //add by ji
                        break;
                    default:
        ...
        private int RenderBlock(int leftPos, int rightPos, Color? selectedTextColor, Color bgColor, IntRect rectContent, IFastGridCellBlock block, FastGridCellAddress cellAddr, bool leftAlign, bool isHoverCell)
        {
            ...
            switch (block.BlockType)
            {
                ...
                case FastGridBlockType.Image:
                    ...
                    break;
                case FastGridBlockType.ColorText:
                    var colorFont = GetFont(block.IsBold, block.IsItalic);
                    int colorTextHeight = colorFont.GetTextHeight(block.TextData);
                    width = colorFont.GetTextWidth(block.TextData, _columnSizes.MaxSize) + 100;
                    height = colorTextHeight;
                    top = rectContent.Top + (int)Math.Round(rectContent.Height / 2.0 - colorTextHeight / 2.0);
                    break;
            }
            ...
            switch (block.BlockType)
            {
                case FastGridBlockType.Text:
                    if (renderBlock)
                    {
                        ...
                        //_drawBuffer.FillRectangle(rectContent, Colors.Orange);
                    }
                    break;
                case FastGridBlockType.Image:
                    if (renderBlock)
                ...
                case FastGridBlockType.Color:
                    if (renderBlock)
                    {
                        if (block.FontColor != null) _drawBuffer.FillRectangle(rectContent, block.FontColor.Value);
                    }
                    break;
                case FastGridBlockType.ColorText:
                    var origin = new IntPoint(leftAlign ? leftPos : rightPos - width, top);
                    var textFont = GetFont(block.IsBold, block.IsItalic);
                    _drawBuffer.DrawString(origin.X, origin.Y, rectContent, block.FontColor ?? CellFontColor, UseClearType ? bgColor : (Color?)null,
                        textFont,
                        block.TextData);
                    break;
                default:
