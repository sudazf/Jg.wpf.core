...
namespace Jg.wpf.controls.Customer.FastDataGrid.Controls
{
    partial class FastGridControl
    {
        ...
        public event EventHandler<RightClickEventArgs> RightClickChanged;
        ...
        private void OnChangeSelectedCells(bool isInvokedByUser, int? row, int? col)
        {
            if (SelectedCellsChanged != null) SelectedCellsChanged(this, new SelectionChangedEventArgs(row, col, isInvokedByUser));
        }

        private void OnRightClickCell(int? row, int? col, bool isRowHeader, bool isColumnHeader)
        {
            RightClickChanged?.Invoke(this, new RightClickEventArgs(row, col, isRowHeader, isColumnHeader));
        }
    }
}
